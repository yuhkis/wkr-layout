...
test('storage allowlist drops input, timestamps, unexpected lessons and invalid values',()=>{
 const p=clean({schemaVersion:2,layoutVersion:'2',text:'example',time:123,lessons:{a:{completed:2,bestAccuracy:94,wrong:'example',keys:['x'],time:1},b:{completed:-1,bestAccuracy:100},unknown:{completed:1,bestAccuracy:100}}},['a','b'],'2');
 assert.deepEqual(p,{schemaVersion:2,layoutVersion:'2',lessons:{a:{completed:2,bestAccuracy:94}}});
});
test('separate versions, corruption, arrays and unbounded counters are not restored',()=>{
 for(const input of [null,{},[],{schemaVersion:2,layoutVersion:'old',lessons:{a:{completed:1,bestAccuracy:100}}},{schemaVersion:2,layoutVersion:'2',lessons:{a:{completed:Infinity,bestAccuracy:100}}}])assert.deepEqual(clean(input,['a'],'2').lessons,{});
});
