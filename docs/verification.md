...
## 2026-09-27 — 配列2.0.0-beta.4（ん＝M、っ＝U、N＝いぇ・ぇの段、記号・矢印の廃止）と練習帳0.4.0

- 正本JSONは171規則で、すべてかな（ローマ字で送る規則）です。2.0.0-beta.3から変わらない規則は155件で、並び順も2.0.0-beta.3と同じです。削除は65件（`n-n`・`u-n`・`m-small-tsu`、記号55件・矢印4件、`Q` の後の全角 ，．／ 3件）、追加は16件（`m-n`・`u-small-tsu`・`n-ye` と、行キーの後の `N` の13件）です。
- `python3 scripts/generate.py --check` で配列表・キー配置の図・v2のIMEテーブル・練習の `data.js` の生成一致を確認しました。組み合わせ表に `N` ぇ の列を加え、図の凡例は右端からはみ出さないよう2行に折り返しています（文字の右端は幅728のうち698）。
- Python 11件、Node 9件が成功しました。全教材の綴りは同じ171規則で再変換して対象かなと一致します。ん を含む問題の打鍵は `M`、っ を含む問題は `U` で示すことを検査しています。
- ローカルに配信したページをChromium系のブラウザで開き、版表示「練習 0.4.0 / 配列 2.0.0-beta.4」、キーボード図の `U`＝っ・`M`＝ん・`N`＝いぇ、課題が11件で6番目が「や・よ・ゆ・いぇの列」であることを確認しました。
  QWERTY体験の体験欄で `E P` `E U` `E M` `S N` `E N` `Q Q` `M` `U` `N` が「きゃかっかんしぇきぇぁぁんっいぇ」になりました。コンソールのエラーは0件です。
- 公開ガードの合成データによる検査と、公開前の全履歴・GitHub上の全表面の監査は、公開操作の記録（Git管理外）に残します。
